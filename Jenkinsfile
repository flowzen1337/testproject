...
def recordStartTime(stageName) {
    ...
    sh "echo 'stage_start_time_seconds{name=\"$stageNameValue\"}  ${currentTime}' | curl -X POST -H 'Connection: close' -H 'Content-Type: text/plain' --data-binary @- ${env.OTEL_EXPORTER_OTLP_ENDPOINT}/metrics"
}
...
def recordEndTime(stageName) {
    ...
    def stageNameValue = "${stageName}"
    def currentTime = sh(script: 'date +%s.%N', returnStdout: true).trim()
    sh "echo 'stage_end_time_seconds{name=\"${stageNameValue}\"} ${currentTime}' | curl -X POST -H 'Connection: close' -H 'Content-Type: text/plain' --data-binary @- ${env.OTEL_EXPORTER_OTLP_ENDPOINT}/metrics"
}
